...
from app.core.deploy.python.utils import argsparser, Timer, get_current_memory_mb
    ...
    def predict_image(self,
        ...
        for i in range(batch_loop_cnt):
            ...
            if run_benchmark:
                ...
                # preprocess
                self.det_times.preprocess_time_s.start()
                print("开始preprocess：" + str(self.det_times.preprocess_time_s.st))
                inputs = self.preprocess(batch_image_list)
                self.det_times.preprocess_time_s.end()
                print("结束preprocess：" + str(self.det_times.preprocess_time_s.et))
                print("preprocess用时：" + str(self.det_times.preprocess_time_s.value()))

                # model prediction
                self.det_times.inference_time_s.start()
                print("开始inference：" + str(self.det_times.inference_time_s.st))
                result = self.predict()
                self.det_times.inference_time_s.end()
                print("结束inference：" + str(self.det_times.inference_time_s.et))
                print("inference用时：" + str(self.det_times.inference_time_s.value()))

                # postprocess
                self.det_times.postprocess_time_s.start()
                print("开始postprocess：" + str(self.det_times.postprocess_time_s.st))
                result = self.postprocess(inputs, result)
                self.det_times.postprocess_time_s.end()
                print("结束postprocess：" + str(self.det_times.postprocess_time_s.et))
                print("postprocess用时：" + str(self.det_times.postprocess_time_s.value()))
                self.det_times.img_num += len(batch_image_list)
                print("=========================================================")
                print("总用时：")
                self.det_times.info()
                print("=========================================================")
                self.det_times.preprocess_time_s.reset()
                self.det_times.inference_time_s.reset()
                self.det_times.postprocess_time_s.reset()
                if visual:
                    visualize(
                        batch_image_list,
                        result,
...
def visualize(image_list, result, labels, output_dir='output/', threshold=0.5):
    ...
    for idx, image_file in enumerate(image_list):
        ...
        warped_result_filename = os.path.join(output_dir, f"warped_result_{timestamp}.jpg")
        img_name = "warped_result_{timestamp}.jpg"
        if not os.path.exists(output_dir):
            os.makedirs(output_dir)
        out_path = warped_result_filename
...
def init_detector_car():
    if FLAGS.use_fd_format:
        deploy_file = os.path.join(FLAGS.model_dir, r'inference.yml')
    else:
        deploy_file = os.path.join(FLAGS.model_dir, r'infer_cfg.yml')
    with open(deploy_file) as f:
        yml_conf = yaml.safe_load(f)
    arch = yml_conf['arch']
    ...
    return detector, FLAGS


paddle.enable_static()
parser = argsparser()
FLAGS = parser.parse_args()
# FLAGS.model_dir = r"..\models\solov2_r50_fpn_1x_coco"
FLAGS.model_dir = r"E:\JJH\dhhi-insightcore\models\FrameCar\mask_rtdetr_hgnetv2_l_6x_coco"
FLAGS.image_file = r"F:\DHHI-PaddleSeg\DHHI-ELSeg-DataSet\JHM_001\JHM_001\1.jpg"
print_arguments(FLAGS)
FLAGS.device = FLAGS.device.upper()
assert FLAGS.device in ['GPU', 'CPU', 'XPU', 'NPU'
