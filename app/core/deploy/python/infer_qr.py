...
from app.core.deploy.python.utils import argsparser, Timer
...
def visualize(image_list, results, labels, output_dir='output/', threshold=0.5):
    ...
    for idx, image_file in enumerate(image_list):
        ...
        im_results = {}
        if 'boxes' in results:
            im_results['boxes'] = results['boxes'][start_idx:start_idx + im_bboxes_num, :]
        if 'masks' in results:
            im_results['masks'] = results['masks'][start_idx:start_idx + im_bboxes_num, :]
        if 'segm' in results:
            im_results['segm'] = results['segm'][start_idx:start_idx + im_bboxes_num, :]
...
def decodeDisplay(image):
    ...
    w = barcode.position.bottom_right.x - x
    h = barcode.position.bottom_right.y - y
    rects_list.append((x, y, w, h))

    # 条形码数据为字节对象，所以如果我们想在输出图像上画出来，就需要先将它转换成字符串
    barcodeData = barcode.text
    ...
    return image, rects_list, QR_info
    ...
    for i in range(batch_loop_cnt):
        ...
        if 'boxes' in results and len(results['boxes']) > 0:
            for dt in results['boxes']:
                ...
                xmax = int(xmax) + 50
                ymax = int(ymax) + 50
                print('class_id:{:d}, confidence:{:.4f}, left_top:[{:.2f},{:.2f}],right_bottom:[{:.2f},{:.2f}]'.format(int(clsid), score, xmin, ymin, xmax, ymax))
                ...
                # cv2.imwrite('./output/1.jpg', roi)
                # det = cv2.QRCodeDetector()
                # info, pts, st_code = det.detectAndDecode(roi)

                im, rects_list, QR_info = decodeDisplay(gray)

                # 把检测到二维码的信息再绘制到BGR彩色图像上
                for data in zip(rects_list, QR_info):
                    x, y, w, h = data[0]
                    text = data[1]
                    cv2.putText(img, text, (x + xmin, y + ymin - 10), cv2.FONT_HERSHEY_SIMPLEX, .5, (0, 0, 125), 2)
                    return img, text, bbox
        savepath = './output/' + os.path.split(batch_image_list[0])[1]
...
# FLAGS.image_dir='./imgs'
FLAGS.model_dir = r'F:\20240801-DHHI-InsightCore\models\qr_models'
FLAGS.image_file = r'F:\20240801-DHHI-InsightCore\app\core\deploy\python\imgs\ocr.jpg'
